...
### 권한 (테스트 분리 원칙)
- **구현 파일만 쓰기 가능**: `app/**`, `components/**`, `lib/**`, `supabase/migrations/**`, `next.config.*`, `package.json`, `README.md`, `CLAUDE.md`(사실 영역만 — 아래 참고) 등
- **테스트 파일은 읽기 전용**: `tests/**`, `e2e/**`, `vitest.config.ts`, `playwright.config.ts` — 수정 금지
- **메타 정책 문서는 읽기 전용**: `AGENTS.md` 전체, `CLAUDE.md` 의 정책 영역(§1, §2, §5, §8, §9, §10, §12, §13, §14) — 수정 금지. 이건 Lead 가 `/meta` 흐름으로 별도 처리한다
...
| 섹션 | 갱신 트리거 | 예시 |
|---|---|---|
| **§3 기술 스택** | 새 라이브러리/도구 도입·교체·제거 | `package.json` 의존성 추가 |
| **§4 디렉토리 구조** | 새 최상위 폴더 생성, 기존 폴더 의미 변경 | `tests/`, `e2e/` 첫 도입 |
| **§6 데이터 모델** | 테이블/컬럼 추가·삭제, 마이그레이션 | `supabase/migrations/NNNN_*.sql` 짝 |
| **§7 라우팅 / 사이트맵** | 새 라우트 추가·제거·이동 | `/friends/invites` 같은 신규 |
| **§11 환경 변수** | 새 env 추가·이름 변경·제거 | `OPERATOR_EMAIL` CSV 확장 같은 시점 |

정책 영역(§1 컨셉 / §2 핵심 가치 / §5 디자인 / §8 코딩 컨벤션 / §9 워크플로우 / §10 브랜치 / §12 결정 로그 운영 / §13 금지 / §14 참고)은 **절대 손대지 않는다**. 운영 정책이라 Lead 가 `/meta` 흐름으로 별도 PR 한다.
...
2. **README.md** 의 "주요 기능", "기술 스택", "사이트맵", "데이터 모델", "디자인 톤" 섹션과 대조
3. **CLAUDE.md 사실 영역** 과 대조:
   - §3 기술 스택 — `package.json` diff 또는 새 도구 도입 시
   - §4 디렉토리 구조 — 새 최상위 폴더 시
   - §6 데이터 모델 — `supabase/migrations/**` 추가 시
   - §7 라우팅 / 사이트맵 — `app/**/page.tsx` 신규/삭제 시
   - §11 환경 변수 — 새 env 참조 또는 제거 시
4. 거짓으로 만든 문장이 있으면 README.md / CLAUDE.md 를 같은 커밋(또는 추가 커밋)에서 갱신
5. **AGENTS.md 와 CLAUDE.md 정책 영역**(§1/§2/§5/§8/§9/§10/§12/§13/§14)은 손대지 마라 — 필요하면 Lead 에 `SendMessage` 로 보고. Lead 가 자율 판단해 `/meta` 흐름으로 별도 처리
6. 갱신이 필요 없으면 그대로 진행
...
- **CLAUDE.md 사실 영역(§3 스택 / §4 디렉토리 구조 / §6 데이터 모델 / §7 사이트맵 / §11 환경 변수) 동기화 누락**
- **AGENTS.md 또는 CLAUDE.md 정책 영역(§1/§2/§5/§8/§9/§10/§12/§13/§14) 임의 수정** — Lead 가 `/meta` 흐름으로 별도 처리
