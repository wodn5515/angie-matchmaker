...
import { saveAnswerAction, submitSurveyAction } from "../actions";
...
export function ChapterRunner({
  ...
  // Required completeness for this chapter
  ...
  // Soft completeness for this chapter (informational)
  ...
  const saveAnswer = async (questionId: string, value: unknown) => {
    const res = await saveAnswerAction({ token, questionId, value });
    if (res.ok) setSavedAt(Date.now());
  };

  // Debounced save for text answers. We track pending text values so we can
  // flush them when leaving the chapter (next/submit), avoiding loss of
  // un-debounced typing.
  ...
  const onNext = () => {
    ...
    // Show chapter result if defined; else jump
  ...
  const goNext = () => {
    if (isLastChapter) {
      ...
      startSubmit(async () => {
        ...
        const res = await submitSurveyAction(token);
        if (!res.ok) {
          if (res.reason === "missing_required") {
            setSubmitError("아직 필수 응답이 빠진 문항이 있어요.");
          } else if (res.reason === "completed") {
            router.replace(`/s/${token}/done`);
          } else {
            setSubmitError("제출에 실패했어요. 잠시 후 다시 시도해주세요.");
          }
          return;
        }
        router.replace(`/s/${token}/done`);
      ...
      // Flush pending text saves before navigating to next chapter.
      startSubmit(async () => {
        await flushPendingTextSaves();
        router.push(`/s/${token}/${chapterIndex + 1}`);
      });
    }
  };
  ...
  return (
    <main className="mx-auto max-w-md px-5 py-8">
      ...
      <div className="mt-8">
        {submitError ? (
          <p className="mb-2 text-center text-xs text-[var(--color-danger)]">
            {submitError}
          </p>
        ) : null}
        <Button
      ...
      <p className="mt-4 text-center text-[11px] text-[var(--color-fg-subtle)]">
        답하면서 자동 저장돼. 친구 {friendName}, 천천히 답해도 괜찮아 💞
      </p>
    </main>
  );
}
  ...
  return (
    <span
      className={cn(
        ...
        show ? "opacity-100 text-[var(--color-success)]" : "opacity-50 text-[var(--color-fg-muted)]",
      )}
    >
...
function ChapterResult({
  ...
  return (
    <main className="mx-auto max-w-md px-5 py-12 text-center">
      ...
      <div className="mt-6 rounded-2xl border border-pink-500/30 bg-pink-500/10 px-5 py-6 text-sm text-fg whitespace-pre-wrap">
        {body}
      </div>
      <div className="mt-6">
